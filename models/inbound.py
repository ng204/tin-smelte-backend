...
class InboundBase(BaseModel):
    ...
    model_config = ConfigDict(
        ...
        json_schema_extra={
            "example": inbound_example_data
        }
    )


class Inbound(SQLModel, InboundBase, table=True):
    id: int = SqlField(default=None, primary_key=True, index=True, unique=True, description="id")
    time: datetime = SqlField(default_factory=datetime.now, description="入库时间")


# class InboundCreate(InboundBase):
#     model_config = ConfigDict(
#         json_schema_extra={
#             "examples": InboundBase.model_config["json_schema_extra"]["examples"]
#         }
#     )
#
#
...
class InboundRead(InboundBase):
    ...
    time: FormattedDateTime = Field(description="入库时间")
    model_config = ConfigDict(
        populate_by_name=True,
        # from_attributes=True,
        json_schema_extra={
            "example": {**{"id": 1, "time": "2023-08-01 10:00:00"}, **inbound_example_data}
        }
    )


# 分页响应模型
class InboundPagination(BaseModel):
    total: int
    items: list[InboundRead]
